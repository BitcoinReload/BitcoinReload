...
const char * const BITCOINRELOAD_CONF_FILENAME = "bitcoinreload.conf";
const char * const BITCOINRELOAD_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/bitcoinreload/bitcoinreload/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bitcoinreload-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", BITCOINRELOAD_SETTINGS_FILENAME);
...
#else
    const char* pszModule = "bitcoinreload";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows: C:\Users\Username\AppData\Roaming\BitcoinReload
    // macOS: ~/Library/Application Support/BitcoinReload
    // Unix-like: ~/.bitcoinreload
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitcoinReload";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/BitcoinReload";
#else
    // Unix-like
    return pathRet / ".bitcoinreload";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", BITCOINRELOAD_CONF_FILENAME);
