...
static void SetupBitcoinReloadTxArgs(ArgsManager &argsman)
{
...
{
    SetupBitcoinReloadTxArgs(gArgs);
    std::string error;
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " bitcoinreload-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  bitcoinreload-tx [options] <hex-tx> [commands]  Update hex-encoded bitcoinreload transaction\n" +
            "or:     bitcoinreload-tx [options] -create [commands]   Create hex-encoded bitcoinreload transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded bitcoinreload transaction
