# PSBT Howto for BitcoinReload Core

Since BitcoinReload Core 0.17, an RPC interface exists for Partially Signed BitcoinReload
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/bitcoinreload/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for BitcoinReload transactions that are not fully signed
yet, together with relevant metadata to help entities work towards signing it.
It is intended to simplify workflows where multiple parties need to cooperate to
produce a transaction. Examples include hardware wallets, multisig setups, and
[CoinJoin](https://bitcoinreloadtalk.org/?topic=279249) transactions.

### Overall workflow

Overall, the construction of a fully signed BitcoinReload transaction goes through the
following steps:
...
- An **Extractor** produces a valid BitcoinReload transaction (in network format)
...
#### Multisig with multiple BitcoinReload Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
BitcoinReload Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `bitcoinreload-cli` is used.
...
Later, when *V* BTCRE has been received on *Amulti*, and Bob and Carol want to
