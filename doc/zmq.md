...
The BitcoinReload Core daemon can be configured to act as a trusted "border
router", implementing the bitcoinreload wire protocol and relay, making
...
The ZeroMQ feature in BitcoinReload Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building bitcoinreloadd:
...
For instance:
    ...
    $ bitcoinreloadd -zmqpubhashtx=tcp://127.0.0.1:24661 \
               -zmqpubhashtx=tcp://192.168.1.2:24661 \
               -zmqpubrawtx=ipc:///tmp/bitcoinreloadd.tx.raw \
...
These options can also be provided in bitcoinreload.conf.
...
From the perspective of bitcoinreloadd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into bitcoinreloadd directly. Furthermore, no information is
...
using. BitcoinReloadd appends an up-counting sequence number to each
