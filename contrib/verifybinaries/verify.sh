...
###   bitcoinreloadcore.org and bitcoinreload.org and compares them.
...
WORKINGDIR="/tmp/bitcoinreload_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bitcoinreloadcore.org"
HOST2="https://bitcoinreload.org"
BASEDIR="/bin/"
VERSIONPREFIX="bitcoinreload-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitcoinreload-',
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "bitcoinreload.org failed to provide signature file, but bitcoinreloadcore.org did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitcoinreload.org and bitcoinreloadcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the BitcoinReload Core binary release signing key installed?"
   fi
